(ns imf.main
  "Kotodama WASM entrypoint for the Imf clean-room actor (L4) — Clojure port.
  ...
  facts (`imf.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"imf.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "imf-compat" :tier tier :entities entities} 200])
